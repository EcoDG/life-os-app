# Life OS — 에너지 기반 개인 운영 시스템

> **"오늘 내 에너지로 할 수 있는 최선을 하자"**

삼성헬스 등급(매우 좋음/좋음/보통/관심 필요)을 기반으로 하루를 설계하고, 습관·성찰·주간 리포트로 패턴을 파악하는 개인 운영 시스템입니다.
...
| 기능 | 설명 |
|------|------|
| **Daily Check-in** | 삼성헬스 4단계 에너지/수면 등급 선택 → 자동 Todo 모드 결정 |
| **Micro Todo** | 에너지 레벨에 따라 1~3개 핵심 목표만 관리 |
| **습관 트래킹** | Slot A/B/C 3개 습관 일일 체크 |
| **Writing Review** | 프리라이팅 → AI 관점 전환 질문 → 인사이트 추출 |
| **개선 포인트** | 반복 패턴 추적 및 현재 실험 중인 개선사항 관리 |
| **Weekly Report** | 에너지 트렌드, 등급 분포 차트, 습관 달성률 주간 요약 |
| **Health Sync API** | `POST /api/health/sync` — 웨어러블 기기에서 건강 데이터 자동 수신 |
| **GitHub 연동** | 일일 성찰 내용을 GitHub 레포지토리에 자동 커밋 |
...
- **AI**: Anthropic Claude (Writing Review, 주간 요약)
...
- MySQL 8.0+ 또는 TiDB Cloud 계정

### 2. 저장소 클론 및 의존성 설치

```bash
git clone https://github.com/EcoDG/life-os-app.git
cd life-os-app
pnpm install
```
...
`.env` 파일을 열고 아래 필수 값을 채워주세요:

| 변수명 | 설명 | 필수 |
|--------|------|------|
| `DATABASE_URL` | MySQL 연결 문자열 | ✅ |
| `JWT_SECRET` | 세션 서명 시크릿 (32자 이상) | ✅ |
| `VITE_APP_ID` | Manus OAuth 앱 ID | ✅ |
| `ANTHROPIC_API_KEY` | Claude AI API 키 | ✅ |
| `BUILT_IN_FORGE_API_KEY` | Manus 내장 API 키 | Manus 플랫폼 전용 |

> Manus 플랫폼에서 배포 시 `BUILT_IN_FORGE_*`, `OAUTH_*`, `OWNER_*` 변수는 자동 주입됩니다.
...
> **주의**: 이 앱은 Express 서버와 MySQL 데이터베이스가 필요합니다.
> Vercel 단독 배포보다 **Railway** 또는 **Render** 사용을 권장합니다.
> Manus 플랫폼에서 직접 배포하면 DB, 인증, 스토리지가 자동 연결됩니다.

### Vercel + PlanetScale/TiDB 조합으로 배포하는 경우
...
#### 2. `vercel.json` 설정 (루트에 생성)

```json
{
  "version": 2,
  "builds": [
    { "src": "server/_core/index.ts", "use": "@vercel/node" }
  ],
  "routes": [
    { "src": "/(.*)", "dest": "server/_core/index.ts" }
  ]
}
```
...
Vercel 대시보드 → Settings → Environment Variables에서 `.env.example`의 모든 변수를 추가합니다.
...
웨어러블 기기(삼성헬스, Garmin 등)에서 건강 데이터를 자동으로 전송할 수 있습니다.
...
| 필드 | 타입 | 설명 |
|------|------|------|
| `date` | string (YYYY-MM-DD) | 날짜 (필수) |
| `sleep_score` | number (0-100) | 수면 점수 |
| `sleep_duration` | number (분) | 수면 시간 |
| `hrv` | number | 심박변이도 |
| `resting_heart_rate` | number | 안정시 심박수 |
| `energy_grade` | 매우 좋음/좋음/보통/관심 필요 | 삼성헬스 에너지 등급 |
| `sleep_grade` | 매우 좋음/좋음/보통/관심 필요 | 삼성헬스 수면 등급 |

> `energy_grade`/`sleep_grade`가 있으면 `sleep_score` 변환보다 우선 적용됩니다.
...
life-os-app/
├── client/src/
│   ├── pages/          # 페이지 컴포넌트 (DailyCheckin, WeeklyReport 등)
│   ├── components/     # 재사용 UI (SamsungGradeSelector 등)
│   └── lib/            # 유틸리티 (energy.ts 등)
├── server/
│   ├── routers.ts      # tRPC 라우터 (모든 API 엔드포인트)
│   ├── db.ts           # 데이터베이스 헬퍼
│   ├── healthSync.ts   # Health Sync REST API
│   └── _core/          # 프레임워크 코어 (OAuth, LLM 등)
├── drizzle/
│   ├── schema.ts       # 데이터베이스 스키마
│   └── migrations/     # 마이그레이션 SQL 파일
└── db-backup/          # 데이터베이스 백업 (CSV)
```
